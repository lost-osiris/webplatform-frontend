...
export default class EditContainer extends Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    let api
    var stateObj = this.utils.getState('jobrunner.job')
    if (stateObj.job == undefined || (stateObj.job.id != this.props.match.params.id)) {
      this.loading(true)
      ...
      this.utils.request(api).then((data) => {
        data.job.then((jobData) => {
          data.api.then((apiInfo) => {
            let output = {
              job: jobData,
              api: apiInfo,
            }
            this.utils.dispatch('INIT', output, 'jobrunner.job').then(() => {
              this.loading(false)
            })
          })
        })
      })
    } else if (stateObj.api == undefined) {
      this.loading(true)
  ...
  componentDidMount() {
    ...
    if (stateObj.job != undefined && stateObj.api != undefined) {
      this.loading(false)
    }
  }
  ...
  UNSAFE_componentWillUpdate(nextProps, nextState) {
    ...
    if (!nextState.loading && this.job == undefined) {
      var job = stateObj.job
      // this.job = job
      this.api = stateObj.api

      // console.log(job)
      if (job.interval != undefined) {
        this.disabled.showInterval = ''
        job.showInterval = true

        if (job.interval.repeats == 'weekly') {
          this.weekComponent = this.createWeeksList()
        }

        this.checked[job.interval.ends.mode] = true

        if (job.interval.ends.mode == 'occurence' || job.interval.ends.mode == 'ondate') {
          this.disabled[job.interval.ends.mode] = ''
        }

        if (job.interval.ends.mode == 'ondate') {
          job.interval.ends.on = moment(job.interval.ends.on*1000)
        }
      } else {
        job.interval = {
          ends: {}
      ...
      job.showRuntime = true
      this.disabled.showRunTime = ''


      this.searchText = job.api

      // console.log(job.kwargs)
      if (Object.keys(job.kwargs).length > 0) {
        this.params = job.kwargs
        this.paramComponent = this.createParamComponent()
      }

      this.job = job
      this.job.run_time = moment(job.run_time*1000)
  ...
  createParamComponent() {
    var body =  _.map(this.params, (value, k) => {
      return (
        <tr key={ k }>
          <td>{ k }</td>
          <td>{ value }</td>
            ...
            <div className="row">
              <div className="col-lg-12 form-control">
                <input type="checkbox"
                  ...
                  checked={ this.job.interval.days[tmp] } />
              </div>
            </div>
            <br />
            <div className="row">
  ...
  toggleWeekday(weekday) {
    if (this.job.interval.days[weekday]) {
      ...
      // console.log(weekday, this.job.interval.days);
    this.forceUpdate()
  }

  render() {
    if (this.state.loading && this.job == undefined) {
      return <Loading />
    }
